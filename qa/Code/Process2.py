...
class Process:
    ...
    def generateAnswers(self, repo):
        ...
        for i in range(len(predicted)):
            answer_cos.append(predicted.loc[i, 'original_sentences'][predicted.loc[i, 'pred_idx_cos'][0]] +
                              predicted.loc[i, 'original_sentences'][predicted.loc[i, 'pred_idx_cos'][1]])
            answer_euc.append(predicted.loc[i, 'original_sentences'][predicted.loc[i, 'pred_idx_euc'][0]] +
                              predicted.loc[i, 'original_sentences'][predicted.loc[i, 'pred_idx_euc'][1]])
            ques.append(predicted.loc[i, 'questions'])
            cosine.append([predicted.loc[i, 'cosine_sim'][predicted.loc[i, 'pred_idx_cos'][0]],
                           predicted.loc[i, 'cosine_sim'][predicted.loc[i, 'pred_idx_cos'][1]]])
            euc.append([predicted.loc[i, 'euclidean_dis'][predicted.loc[i, 'pred_idx_euc'][0]],
                        predicted.loc[i, 'euclidean_dis'][predicted.loc[i, 'pred_idx_euc'][1]]])
        ...
        df.to_csv(r'../Results/permits2.csv')
